...
//				map_ySI_Icon[tag] = node->data;
	...
	for (auto mIter = (*g_allFormsMap)->Begin(); mIter; ++mIter)
	{
		...
		for (auto& it = map_ySI_List.begin(); it != map_ySI_List.end(); it++) {
			tile = *it;
		...
		for (auto& it = map_ySI_Tag.begin(); it != map_ySI_Tag.end(); it++) {
			tile = *it;
			...
			case 40: {
				if (tile->GetValue(kTileValue_user4)) {
					if ((tile->GetValue(kTileValue_user4)->num != GetWeaponValue(form, eWeap_Skill))) continue;
				}
				if (tile->GetValue(kTileValue_user5)) {
					if ((tile->GetValue(kTileValue_user5)->num != GetWeaponValue(form, eWeap_Type))) continue;
				}
				if (tile->GetValue(kTileValue_user6)) {
					if ((tile->GetValue(kTileValue_user6)->num != GetWeaponValue(form, eWeap_HandGrip))) continue;
				}
				if (tile->GetValue(kTileValue_user7)) {
					if ((tile->GetValue(kTileValue_user7)->num != GetWeaponValue(form, eWeap_AttackAnim))) continue;
				}
				if (tile->GetValue(kTileValue_user8)) {
					if ((tile->GetValue(kTileValue_user8)->num != GetWeaponValue(form, eWeap_ReloadAnim))) continue;
				}
				if (tile->GetValue(kTileValue_user9)) {
					if ((tile->GetValue(kTileValue_user9)->num != GetWeaponValue(form, eWeap_IsAutomatic))) continue;
				}
				if (tile->GetValue(kTileValue_user10)) {
					if ((tile->GetValue(kTileValue_user10)->num != GetWeaponValue(form, eWeap_HasScope))) continue;
				}
				if (tile->GetValue(kTileValue_user11)) {
					if ((tile->GetValue(kTileValue_user11)->num != GetWeaponValue(form, eWeap_IgnoresDTDR))) continue;
				}
				...
				if (tile->GetValue(kTileValue_user13)) {
					if ((tile->GetValue(kTileValue_user13)->num > GetWeaponValue(form, eWeap_NumProj))) continue;
				}
				if (tile->GetValue(kTileValue_user14)) {
					if ((tile->GetValue(kTileValue_user14)->num != GetWeaponValue(form, eWeap_SoundLevel))) continue;
				}
...
inline signed int __fastcall CompareItemsWithTags(ContChangesEntry* a2, ContChangesEntry* a1)
{
...
inline void __fastcall SetStringValueTagImage(Tile* tile, ContChangesEntry* entry, enum TileValues tilevalue, char* src, char propagate)
{
...
inline void __fastcall SetStringValueTagRose(Tile* tile, ContChangesEntry* entry, enum TileValues tilevalue, char* src, char propagate)
{
...
inline __declspec(naked) void Tile__SetStringValueHotkeyHook() {
	static const auto SetStringValue = UInt32(SetStringValueTagImage);
...
inline __declspec(naked) void Tile__SetStringValueHotkeyHook2() {
	static const auto SetStringValue = UInt32(SetStringValueTagRose);
...
inline __declspec(naked) void InventoryMenuSortingHook()
{
...
inline __declspec(naked) void BarterContainerMenuSortingHook()
{
	static const auto CompareItems = reinterpret_cast<UInt32>(CompareItemsWithTags);
	_asm
	{
