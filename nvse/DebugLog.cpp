...
char				IDebugLog::formatBuf[32768] = { 0 };
uint32_t			IDebugLog::indentLevel = 0;
...
void IDebugLog::Open(const char* path)
{
	...
	if (!logFile)
	{
		...
		do
		{
			our_snprintf(name, sizeof(name), "%s%d", path, id);
			id++;
...
void IDebugLog::OpenRelative(int folderID, const char* relPath)
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, folderID, NULL, SHGFP_TYPE_CURRENT, path))) {
...
void IDebugLog::Message(const char* message, const char* source)
{
	...
	if (inBlock)
	{
		SeekCursor(indentLevel + strlen(headerText));
	}
	else
	{
		SeekCursor(indentLevel);
...
void IDebugLog::FormattedMessage(const char* fmt, ...)
{
	...
	our_vsnprintf(formatBuf, sizeof(formatBuf), fmt, argList);
	Message(formatBuf);
	va_end(argList);
}
...
{
	our_vsnprintf(formatBuf, sizeof(formatBuf), fmt, args);
	Message(formatBuf);
}

void __fastcall IDebugLog::Log(LogLevel level, const char* fmt, va_list args)
{
...
void IDebugLog::SetIndent(uint32_t indent) {
	indentLevel = indent;
}

uint32_t IDebugLog::GetIndent() {
	return indentLevel;
}
