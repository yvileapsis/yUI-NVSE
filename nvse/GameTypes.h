...
public:
	...
	_Node* GetLastNode(SInt32* outIdx = nullptr) const
	{
		SInt32 index = 0;
		_Node* node = Head();
		while (node->next)
	...
	public:
		...
		Iterator(_Node* node = nullptr) : m_curr(node) {}
	...
	Item* GetFirstItem() const { return m_listHead.data; }
...
struct BSSimpleArray
{
	...
	virtual T* Allocate(UInt32 size);
