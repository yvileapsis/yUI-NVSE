...
void Tile::Dump()
{
	PrintLog("%08X\t%s", this, name.m_data);
	s_log.Indent();
	...
	for (BSSimpleArray<Value*>::Iterator iter(values); !iter.End(); ++iter)
	{
		...
		if (value->str)
			PrintLog("%d  %s: %s", value->id, traitName, value->str);
		/*else if (value->action)
			PrintLog("%d  %s: Action %08X", value->id, traitName, value->action);*/
		else
			PrintLog("%d  %s: %.4f", value->id, traitName, value->num);
	}
