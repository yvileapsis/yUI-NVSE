...
public:
	...
	static void			SetIndent(uint32_t indent);
	static uint32_t 	GetIndent();
...
private:
	...
	static char			formatBuf[32768];	//!< temp buffer used for formatted messages

	static uint32_t		indentLevel;		//!< the current indentation level
...
inline void __fastcall _FATALERROR(const char* fmt, ...)
{
	va_list args;
...
inline void __fastcall _ERROR(const char* fmt, ...)
{
	va_list args;
...
inline void __fastcall _WARNING(const char* fmt, ...)
{
	va_list args;
...
inline void __fastcall _MESSAGE(const char* fmt, ...)
{
	va_list args;
...
inline void __fastcall _VMESSAGE(const char* fmt, ...)
{
	va_list args;
...
inline void __fastcall _DMESSAGE(const char* fmt, ...)
{
	va_list args;
...
public:
	AutoIndent() {
		indent = IDebugLog::GetIndent();
		IDebugLog::SetIndent(indent + 1);
	}
	~AutoIndent() {
		IDebugLog::SetIndent(indent);
	}
};
