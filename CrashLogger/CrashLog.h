...
template <typename T>
inline _RTTICompleteObjectLocator* GetCompleteObjectLocator(const T* ptr) // not have vbtable
{
	ptrdiff_t offset = 0;
	...
	return (_RTTICompleteObjectLocator*)(ReadData<size_t>(
		ReadData<size_t>((size_t)ptr + offset) - 4));
}

template <typename T>
inline TypeDescriptor* __RTtypeid(T* ptr)
{
...
inline std::string printTypeName(void* object) {
	try {
		auto typeInfo = WrappedRTTypeID(object);
