...
{
	bool __fastcall GetRegistryString(HKEY key, const char* name, char* buffer, SIZE_T bufferSize)
	{
		...
		try {
			...
			char release[16];
			{
				...
				if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_READ, &key) == ERROR_SUCCESS) {
					...
					UInt32 buildNumberInt = std::atoi(buildNumber);
			...
			_MESSAGE("OS:  \"%s - %s (%s)\"", version, buildNumber, release);
			_MESSAGE("CPU: \"%s\"", cCPU);
			_MESSAGE("GPU: %s", gpu); // Already has quotes
			_MESSAGE("RAM: \"%.2f GB\"", memAmount / 1024.f / 1024.f);

		}
		catch (...) { 
			_MESSAGE("Failed to print device info.\n");
		}
	}
}