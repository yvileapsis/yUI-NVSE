...
CrashLogger::Handle::_Handler lastHandler = nullptr;
...
public:
	...
	UInt32 address;
	UInt32 size;
	CrashLogger::Handle::_Handler function;
	std::string name;

	Label(UInt32 address, CrashLogger::Handle::_Handler function = lastHandler, Type type = kType_RTTIClass, std::string name = "", UInt32 size = 4)
...
std::vector setOfLabels {
	...
	Label(0x011F3374, CrashLogger::Handle::AsUInt32, Label::kType_Global, "TileValueIndirectTemp"),

	Label(kVtbl_Menu, CrashLogger::Handle::AsMenu),
	...
	Label(kVtbl_Tile, CrashLogger::Handle::AsTile),
	Label(kVtbl_TileMenu),
	Label(kVtbl_TileRect),
	Label(kVtbl_TileImage),


	Label(kVtbl_TESForm, CrashLogger::Handle::AsTESForm),
	...
	Label(kVtbl_TESObjectREFR, CrashLogger::Handle::AsTESObjectREFR),
	...
	Label(kVtbl_BaseProcess, CrashLogger::Handle::AsBaseProcess),
	...
	Label(kVtbl_TESTexture, CrashLogger::Handle::AsTESTexture),
	Label(kVtbl_TESIcon),
	Label(kVtbl_QueuedTexture, CrashLogger::Handle::AsQueuedTexture),


	// Ni

	Label(kVtbl_NiNode, CrashLogger::Handle::AsNiObjectNET),
	Label(kVtbl_BSFadeNode),


	// animations
	Label(kVtbl_BSAnimGroupSequence, CrashLogger::Handle::AsBSAnimGroupSequence),
	Label(kVtbl_AnimSequenceSingle, CrashLogger::Handle::AsAnimSequenceSingle),

	// model
	Label(kVtbl_BSFile, CrashLogger::Handle::AsBSFile),
	Label(kVtbl_TESModel, CrashLogger::Handle::AsTESModel),
	Label(kVtbl_QueuedModel, CrashLogger::Handle::AsQueuedModel),
...
namespace CrashLogger::YvilesMagicBox
{
	...
	std::string DecryptPtr(UInt32 ptr, UInt32 depth = 5)
	{
		try
		{
			...
			{
				if (std::string onetime = CrashLogger::NVVtables::GetStringForVTBL((void*)ptr, vtbl); !onetime.empty()) return full + onetime;
...
namespace CrashLogger::Stack
{
	void Get(EXCEPTION_POINTERS* info)
	{
		...
		for (unsigned int i = 0; i < ce_printStackCount; i += 1) {
			...
			if (i <= 8)
			{
				Log() << FormatString("%2X | 0x%08X | %s", i, esp[i], CrashLogger::YvilesMagicBox::DecryptPtr(esp[i]).c_str());
			}
			else
			{
				std::string result = CrashLogger::YvilesMagicBox::DecryptPtr(esp[i], 0);
